...
return new class extends Migration
{
    ...
    {
        Schema::connection($this->connection)->create('bread_boost_archive', function (Blueprint $table) {
            ...
            $table->unique(['franchise_store', 'business_date'], 'unique_bread_boost_archive');
            $table->index(['franchise_store', 'business_date']);
        });

        // Add monthly partitioning with compression
        $this->addMonthlyPartitions('bread_boost_archive');
    }
    ...
    {
        Schema::connection($this->connection)->dropIfExists('bread_boost_archive');
    }
};
